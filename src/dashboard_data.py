...
Loads every stage of the pipeline and unifies it into one in-memory structure
that both the static HTML report (``report_builder.py``) and the Streamlit app
(``dashboard.py``) consume.

It reads the committed artifacts:

  pose estimation   outputs/presenter_keypoints.csv
  tracking          outputs/{clip}_coverage_stats.json
                    outputs/{clip}_aggregate_heatmap.png
                    outputs/{clip}_movement_path.png
  body language     outputs/body_language_metrics.csv
  gesture model     models/gesture_detector_best.pt
                    outputs/gesture_metrics_summary.json

The dashboard renders from the keypoints, coverage, and body-language artifacts
alone (all committed), so it works from a clean clone. The gesture-model card
degrades gracefully to ``None`` when its summary is not present.
"""
...
# COCO-17 keypoint order. Kept here so the dashboard can label keypoints without
# importing the heavier pose module.
KEYPOINT_NAMES = [
...
# One fixed colour per metric family, used identically in the HTML cards, the
# matplotlib timeline, and the Streamlit app so a colour always means the same
# metric across the whole product.
METRIC_COLORS = {
    "posture": "#2b8a8a",   # teal
...
class ClipReport:
    ...
    # Raw coverage block, kept for anyone who wants the detail
...
def _read_metrics(outputs_dir: Path) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"body_language_metrics.csv not found at {path}. Run "
            "notebooks/03_body_language_metrics.ipynb and commit the CSV first."
        )
...
def _find_model_summary(outputs_dir: Path, models_dir: Path) -> dict | None:
    """Locate the gesture-model metrics summary. It is git-ignored, so try the
    likely locations and filenames."""
    names = ["gesture_metrics_summary.json", "m4_metrics_summary.json"]
    folders = [outputs_dir, outputs_dir / "m4_custom_training", models_dir]
...
def build_report_data(outputs_dir: str | Path = "outputs",
    ...
    -------
    ReportData
        clips (one ClipReport each), the gesture-model card (or None), and flags.
    """
    ...
    return ReportData(
        clips=clips,
        model_card=_find_model_summary(outputs_dir, models_dir),
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(data.clips)} clips | "
          f"model card: {'yes' if data.model_card else 'not present'}")
    for c in data.clips:
