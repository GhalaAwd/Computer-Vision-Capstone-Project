...
Turns the integrated dataset from ``dashboard_data.build_report_data()`` into a
single self-contained HTML report: summary cards, a synced per-second delivery
timeline, and the spatial-coverage visuals, per clip, plus the gesture-model card.
...
companion; this file is the shareable artifact.
...
def delivery_timeline_fig(clip: ClipReport):
    ...
    # a quiet, honest note when posture was never measurable
    if np.isnan(stability).all():
        ax_left.text(0.5, 0.5, "posture not confidently measurable in this clip",
                     transform=ax_left.transAxes, ha="center", va="center",
                     fontsize=9, color="#9aa5b1", style="italic")
...
def _model_section(model_card: dict | None) -> str:
    ...
    <section class="model absent">
      ...
      <p class="sub">The gesture-model summary (<code>gesture_metrics_summary.json</code>)
      and weights (<code>gesture_detector_best.pt</code>) are git-ignored. Add the summary
      to <code>outputs/</code> before the final run and this card fills in automatically.</p>
    </section>"""
...
<body><div class="wrap">
  ...
  <div class="foot">Generated {stamp} &middot; MentorVision &middot; built from the
